...
class PathManager {
  public:
    ...
    void addPath(t_coord startPos, t_coord endPos, Agent* target, AgentType agentType, bool draw = false, PathType type = PathType::A_Pre_Manhattan);
    void applyDoor(Door* door);
    void debug(bool d) { _debug = d; }
    void reset() {
        _mutex.lock();
        while (!_pathQueue.empty()) _pathQueue.pop();
        _mutex.unlock();
    }
  private:
